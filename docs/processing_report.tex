...
\section{Example CTD configuration (\code{config\_ctd.json})}

The reference deployment (NOPP-Aleutians). Every quantity discussed in Section~2 is set here;
paths may use \code{\textasciitilde} and are overridable by environment variables.
...
\section{Example ADCP configuration (\code{config\_adcp.json})}

The reference template (TLC 2023). Every quantity discussed in Section~3 is set here; the
instrument geometry is not, since it is read from the \code{.ad2cp}.
