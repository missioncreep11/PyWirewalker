...
\item \textbf{v2} --- a buoyant-ascent model in which the vertical velocity is taken from
$dp/dt$, the horizontal platform motion is depth-gated, and the tilt is derived from
low-passed accelerometer data. It is immune to AHRS attitude faults and applies a
\textbf{sail correction} (ported from the reference code) for the along-wire travel of the
vehicle on an inclined mooring line.
...
\textbf{Motion-immune shear (ported).} In addition to gridded velocity, the L2 product
carries a \textbf{beam-differenced vertical shear} (\code{shearE}, \code{shearN}), ported from
...
Motion correction & Single AHRS-based model & Selectable v1/v2 models with \code{auto} attitude reconstruction, robust to AHRS faults \\
